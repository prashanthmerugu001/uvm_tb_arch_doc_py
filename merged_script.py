...
path = input("enter path of TB directory: ")
...
print("value of font_size",font_size);
...
#def top():
n = 1;
top_dim = call_simple_rect(w,h,n,"TOP","white","black",img1);
#Create second inner rectangle test n=3
#def test(name):
n = 3;
#label = name
test_dim = call_simple_rect(w,h,n,"TEST","white","black",img1);
#Create third inner rectangle env n=5
n = 5;
env_dim = call_simple_rect(w,h,n,"ENV","white","black",img1);
#Create fifth inner rectangle SCOREBOARD
top_right = (w-140,150)
bottom_left = (150,230)
start_x = ((w-140)+400)/2 + 12;
start_y = 180;
draw_rect(img1,(top_right, bottom_left), fill="white" ,color="black", width=10)
...
n = 7;
agnt_cnt = 2 #int(input("Enter no. of agents"))
print("Navn_ag_cnt")
print(agnt_cnt)
w1 = (env_dim/agnt_cnt)
h1 = (env_dim/agnt_cnt)
...
for val in range(agnt_cnt):
    ...
    print(top_right);
    print(bottom_left);
    draw_rect(img1,(top_right, bottom_left), fill="white" ,color="black", width=outline_width)
...
#    y5 = y3 + 100
    ...
    #  PORT CONNECTING SCOREBOARD AND MONITOR
  ...
  #  xdiff_mon = x11 - x10;
  #  ydiff_mon = y10 - y11;
  #  start_x_mon = x10 + xdiff_mon / 20;
  #  start_y_mon = y11 + ydiff_mon / 2;
    outline_width = 10
    draw_rect(img1, (top_right, bottom_left), fill="black", color="black", width=outline_width)
  #  wr_text_in_rect(img1, start_x_mon, start_y_mon, "PORT", "BLACK")

    x10=x10+10;
    y10=y10+10;
    img1.ellipse((x10 - 15, y10 - 15, x10 - 0, y10 - 0), fill="white", outline="black")

    #  PORT CONNECTING SEQUENCER AND DRIVER
   ...
   ## xdiff_mon = x8 - x9;
   ## ydiff_mon = y9 - y8;
   ## start_x_mon = x9 + xdiff_mon / 1;
   ## start_y_mon = y8 + ydiff_mon / 2;
    outline_width = 10
    draw_rect(img1, (top_right, bottom_left), fill="black", color="black", width=outline_width)
   # #wr_text_in_rect(img1, start_x_mon, start_y_mon, "PORT", "BLACK")
    ...
    x9=x9+10;
    y9=y9+10;
    img1.ellipse((x9 - 15, y9 - 15, x9 - 0, y9 - 0), fill="white", outline="black")

    # PORT CONNECTIVITY ON DRIVER
    ...
    # PORT CONNECTIVITY ON MONITOR

    x12 = x1 + (6.5 * xdiff / 20);
    y12 = y1 - (8.8 * ydiff / 20);
    x13 = x0 - (12.6 * xdiff / 20);
    y13 = y0 + (12 * ydiff / 20);
    top_right = (x13, y13);
    bottom_left = (x12, y12);
    outline_width = 10
    ...
    window_name = 'Image'
    #####################DRAW ARROW BETWEEN DRIVER AND INTERFACE WITH RESPECT TO AGENT COUNT#################################
    w = 0;
    h1= 50;
    h2= 170;
    b = agnt_cnt
    for val in range(b):
        print("VALUE OF B",b)
        start_point = (int(start_x_drv - w), int(start_y_drv) + h1)
        # End coordinate
        end_point = (int(start_x_drv - w), int(start_y_drv + 25) + h2)
        color = (0, 0, 0)
        thickness = 3
